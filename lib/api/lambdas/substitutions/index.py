    ...
    if not filters:
        ...
        if not filters.get("category_match_level") == None:
            ...
            for cat in cats_to_match:
                terms.append({"term": {"categories": cat}})

            filters_exp.update({"bool": {"must": terms}})
            ...
            filters_exp.update(
                {
                    "terms_set": {
                        "diet_type": {
                            "terms": diets,
                            "minimum_should_match_script": {"source": f"{d}"},
                        }
                    }
                }
            )
        ...
        if filters.get("brand_match") == True:
            filters_exp.update({"match_phrase": {"brand": product["brand"]}})
            ...
            exp = {
                "script": {
                    "script": {
                        ...
                        """,
                        "lang": "painless",
                        "params": {"subsetTerms": product["allergens"]},
                    }
                }
            }

            if (not filters_exp.get("bool") == None) and (
                not filters_exp["bool"].get("must") == None
            ):
                filters_exp["bool"]["must"].append(exp)

            else:
                filters_exp.update({"bool": {"must": [exp]}})

        if filters.get("custom_filter_script") == True:
            if (not filters_exp.get("bool") == None) and (
                not filters_exp["bool"].get("must") == None
            ):
                filters_exp["bool"]["must"].append(custom_filter_script(product))

            else:
                filters_exp.update({"bool": {"must": [custom_filter_script(product)]}})
...
def handler(event, context):
    ...
    category_match_level = event["queryStringParameters"].get("category_match_level")
    diet_type_match_count = event["queryStringParameters"].get("diet_type_match_count")
    brand_match = event["queryStringParameters"].get("brand_match")
    no_new_allergens = event["queryStringParameters"].get("no_new_allergens")
    custom_filter_script = event["queryStringParameters"].get("custom_filter_script")
    ...
    if not category_match_level == None:
        filters.update({"category_match_level": int(category_match_level)})

    if not diet_type_match_count == None:
        filters.update({"diet_type_match_count": int(diet_type_match_count)})

    if not brand_match == None:
        filters.update({"brand_match": bool(brand_match)})

    if not no_new_allergens == None:
        filters.update({"no_new_allergens": bool(no_new_allergens)})

    if not custom_filter_script == None:
        filters.update({"custom_filter_script": bool(custom_filter_script)})

    neighbours = search(id, product, filters, size)
